...
export class DataControl {
  ...
  private static readObject(data: object, props: string[]): any {
  ...
  static compare(a: any, b: any, p: string): number {
    const d = p.split('.');
  ...
  static remove<T>(target: T, props: string[]): T {
  ...
  static updateItem<T>(list: T[], data: any, prop: string): T[] {
...
export class FileControl {
  static async convertUrlToBase64(url: string): Promise<any> {
  ...
  static async convertFileToBase64(file: File): Promise<any> {
    return new Promise((resolve, reject) => {
      const reader = new FileReader();
      reader.addEventListener('load', () => resolve(reader.result), false);
  ...
  static getExif(imageString: string, group: string, code: number): any {
...
export class FormGroupControl {
  ...
  static touchControls(form: FormGroup): void {
...
export class TimestampControl {
  static toNgbDate(datetime: string): {day: number, month: number, year: number} {
  ...
  static diffDateTimeFromNow(date1: Date, date2: Date = new Date()): {day: number, month: number, year: number} {
  ...
  static translateDateTimeFromNow(date1: Date, date2: Date = new Date()): string {
