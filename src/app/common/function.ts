...
export class DataControl {
    ...
    static sort<T>(data: T[], prop: string, direction: string): T[] {
        ...
        switch (direction) {
            case 'asc': return data.sort((a, b) => this.compare(a, b, prop));
            case 'desc': return data.sort((a, b) => this.compare(b, a, prop));
            default: return data;
        }
    }
    ...
    static updateItem<T>(list: T[], data, prop: string) {
        ...
        if (this.isObject(list[0])) return list;

        let item = list.find(x => x[prop] == data[prop]);
        item = this.cloneProps(data, item, true);
        return list;
    }
    ...
    private static cloneProps<T>(data: T, target: T, includeObject = false): T {
        for (let prop in data) {
            if (data.hasOwnProperty(prop)) {
                if (DataControl.isPrimitive(prop)) {
                    target[prop] = data[prop];
                } else if (DataControl.isObject && includeObject === true) {
                    target[prop] = data[prop];
                }
            }
        }
