...
export class LoginComponent implements OnInit {
  ...
  constructor(private router: Router, private route: ActivatedRoute, private formBuilder: FormBuilder, 
    private service : AuthService, private alertService: AlertService, private sessionService: SessionService) { }
  ...
  clickSummit() {
    ...
    this.service.login(user).subscribe(res => {
      ...
      if (json.role == 'MANAGER') {
        ...
        if (!this.redirect || this.redirect == '') window.location.href = Const.THIS_URL;
        else window.location.href = this.redirect;
      }
    }, err => {
      this.alertService.add("Username or Password mismatched", 'danger');
    });
  }
}
