...
export class StatisticCustomerComponent implements OnInit {
  ...
  private initAgeData(fromDate: string, toDate: string): void {
  ...
  private initGenderData(fromDate: string, toDate: string): void {
    const startTime = this.alertService.startTime();
  ...
  private caclChartTitle(data: IChartData, value: string): IChartData {
    switch (value) {
